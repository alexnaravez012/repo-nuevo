...
@Component({
  selector: 'app-pedidos-domicilios-entregados-n',
  templateUrl: './pedidos-domicilios-entregados-n.component.html',
  styleUrls: ['./pedidos-domicilios-entregados-n.component.css']
})
export class PedidosDomiciliosEntregadosNComponent implements OnInit {
  @Input()
  ...
  updateData(){
    ...
    CPrint("THIS IS MY ID STORE: ",this.id_store)
      //-TIMER-//
      this.intervalUpdate = setInterval(function(main:PedidosDomiciliosEntregadosNComponent){
        main.UpdateTimer();
      },1000,this);
      //-TIMER-//
    }
  ...
  aprobarPedido(element){
    ...
    dialogRef.afterClosed().subscribe(result => {
      CPrint(result)
      if(result.response){
        console.log("NOTAS: ",result.obs)
        this.http2.post(Urlbase.facturacion+"/pedidos/actualizarEstadoPedidoApp?idbillpedidoprov="+element.id_BILL+"&idbillstateclient=704&idbillstateprov=804&notas="+result.obs+"&ESTADOORIGENPROV="+this.SelectedBillState+"&IDTHIRDUSER="+this.locStorage.getToken().id_third+"&ACTOR=P",{}).subscribe(response => {
          this.showNotification('top', 'center', 3, "<h3>El cliente acepto la <b>NOVEDAD</b></h3> ", 'info');
