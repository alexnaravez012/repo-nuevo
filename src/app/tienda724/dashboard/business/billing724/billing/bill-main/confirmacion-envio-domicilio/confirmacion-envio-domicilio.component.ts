...
@Component({
  selector: 'app-confirmacion-envio-domicilio',
  templateUrl: './confirmacion-envio-domicilio.component.html',
  styleUrls: ['./confirmacion-envio-domicilio.component.css']
})
export class ConfirmacionEnvioDomicilioComponent implements OnInit {
  ...
  sendData(data){
    ...
    if(this.id_person){
      ...
      if (this.detailBillDTOList.length > 0) {
        ...
        CPrint("THIS IS BILL DTO V2 WAWA: ", JSON.stringify({idthirdemployee:this.billDTO.id_third,
          idthird:this.billDTO.id_third_employee,
          idbilltype:this.billDTO.id_bill_type,
          notes:this.billDTO.documentDTO.body,
          idthirddestinity:this.data.element.idthirdcliente,
      ...
      if (this.detailBillDTOList.length > 0) {
        ...
        CPrint("THIS IS BILL DTO V2: WEWE", JSON.stringify({idthirdemployee:this.billDTO.id_third,
          idthird:this.billDTO.id_third_employee,
          idbilltype:this.billDTO.id_bill_type,
          notes:this.billDTO.documentDTO.body,
          idthirddestinity:this.data.element.idthirdcliente,
  ...
  loadDetails(){
    ...
    this.http.get(Urlbase.facturacion + "/billing/detail?id_bill="+this.SelectedPedido).subscribe(r => {
      ...
      r.forEach(element => {;
          ...
          console.log(element[3],Number(element[0]))
      });
    })
  }
  ...
  async getInventoryList(store){
    this.httpClient.get(Urlbase.facturacion + "/pedidos/getThirDataPedidos?id_store="+this.locStorage.getIdStore()).subscribe(data => {
      this.domiList = data;
      CPrint("DOMI, Domi");
      CPrint(data);
    });
