...
@Component({
  selector: 'app-editar-pedido-restaurante',
  templateUrl: './editar-pedido-restaurante.component.html',
  styleUrls: ['./editar-pedido-restaurante.component.css']
})
export class EditarPedidoRestauranteComponent implements OnInit {
  ...
  constructor(public router: Router, private datePipe: DatePipe, public thirdService: ThirdService,private http2: HttpClient,private http: HttpClient, public locStorage: LocalStorage,private fb: FormBuilder, private billingService: BillingService,
    public inventoriesService: InventoriesService, public dialogRef: MatDialogRef<EditarPedidoRestauranteComponent>
    ,@Inject(MAT_DIALOG_DATA) public data: any, public dialog: MatDialog, private httpClient: HttpClient) {
  ...
  ngOnInit() {
    ...
    console.log("MAH DATAH: ",this.data)
...
onSearchChange(itemQuant,event){
  ...
  postNewDetails(){
    ...
    this.http.post(Urlbase.facturacion + "/billing/agregarProductosMesa?idbill="+this.data.item.idbill+"&detallepedido="+detailList.substring(0, detailList.length - 1),{}).subscribe(response => {
