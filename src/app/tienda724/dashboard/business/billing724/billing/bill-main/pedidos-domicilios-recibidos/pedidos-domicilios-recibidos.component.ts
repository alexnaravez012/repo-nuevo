...
@Component({
  selector: 'app-pedidos-domicilios-recibidos',
  templateUrl: './pedidos-domicilios-recibidos.component.html',
  styleUrls: ['./pedidos-domicilios-recibidos.component.css']
})
export class PedidosDomiciliosRecibidosComponent implements OnInit {
  @Input()
  ...
  updateDataRecibidos(){
    ...
    //-TIMER-//
    this.intervalUpdate = setInterval(function(main:PedidosDomiciliosRecibidosComponent){
      main.UpdateTimer();
    },1000,this);
    //-TIMER-//
  }
  ...
  postExcel(){
    console.log("URL: ", Urlbase.facturacion + "/pedidos/postExcelDomicilio?date1="+this.date1+"&date2="+this.date2+"&id_store="+this.locStorage.getIdStore());
    this.http2.post(Urlbase.facturacion + "/pedidos/postExcelDomicilio?date1="+this.date1+"&date2="+this.date2+"&id_store="+this.locStorage.getIdStore(),{}, {responseType: 'text'}).subscribe(
      response  => {
        console.log(response);
        window.open(Urlbase.root+"reportes/"+response, "_blank");
      }
    )
  }
  //-TIMER-//
  ...
  UpdateTimer(){
    for(let n = 0;n<this.dataSource.data.length;n++){
      ...
      this.segs[element.id_BILL] = parseInt(((b.getTime() - a.getTime() + 60)/60000)+"")
    }
  }
  //-TIMER-//
