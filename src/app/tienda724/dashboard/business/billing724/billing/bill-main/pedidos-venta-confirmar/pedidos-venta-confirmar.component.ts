...
@Component({
  selector: 'app-pedidos-venta-confirmar',
  templateUrl: './pedidos-venta-confirmar.component.html',
  styleUrls: ['./pedidos-venta-confirmar.component.css']
  })
export class PedidosVentaConfirmarComponent implements OnInit  {
  ...
  quickConfirm(){
    ...
    CPrint(Urlbase.facturacion+ "/pedidos/actualizarPedidoNovedadApp?idbillpedidoprov="+this.data.element.id_BILL+"&nuevosdetalles="+detailList.substring(0, detailList.length - 1)+"&notas="+this.obs+"&idpaymentmethod="+this.paymentMethod+"&ESTADOORIGENPROV="+this.SelectedBillState+"&IDTHIRDUSER="+this.locStorage.getToken().id_third+"&ACTOR=P");
    this.http.post(Urlbase.facturacion+ "/pedidos/actualizarPedidoNovedadApp?idbillpedidoprov="+this.data.element.id_BILL+"&nuevosdetalles="+detailList.substring(0, detailList.length - 1)+"&notas="+this.obs+"&idpaymentmethod="+this.paymentMethod+"&ESTADOORIGENPROV="+this.SelectedBillState+"&IDTHIRDUSER="+this.locStorage.getToken().id_third+"&ACTOR=P",{}).subscribe(item => {
      if(item==1){
        this.showNotification('top', 'center', 3, "<h3>Pedido procesado con <b>NOVEDAD.</b></h3> ", 'info');
