...
@Component({
  selector: 'app-pedidos-domicilios-encamino',
  templateUrl: './pedidos-domicilios-encamino.component.html',
  styleUrls: ['./pedidos-domicilios-encamino.component.css']
})
export class PedidosDomiciliosEncaminoComponent implements OnInit {
  @Input()
  ...
  updateData(){
    ...
    //-TIMER-//
    this.intervalUpdate = setInterval(function(main:PedidosDomiciliosEncaminoComponent){
      main.UpdateTimer();
    },1000,this);
    //-TIMER-//
  }
  ...
  cancelarPedido(element){
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if(result.response){
        ...
        this.http2.post(Urlbase.facturacion+"/pedidos/actualizarEstadoPedidoApp?idbillpedidoprov="+element.id_BILL+"&idbillstateclient=708&idbillstateprov=808&notas=ENTREGADO CON NOVEDAD: "+result.obs+"&ESTADOORIGENPROV="+this.SelectedBillState+"&IDTHIRDUSER="+this.locStorage.getToken().id_third+"&ACTOR=P",{}).subscribe(response => {
          this.showNotification('top', 'center', 3, "<h3>Se ENTREGO su pedido con NOVEDADES con <b>EXITO</b></h3> ", 'info');
          this.mainclassref.SendFCMpush("Pedido Entregado","Tu pedido ha sido 'Entregado con novedades'",{message:"Order Update",id_bill:element.id_BILL,status:708,status_p:808},element.idthirdcliente,element.cliente,element.idapp);
          ...
          this.updateData();
          this.mainclassref.setTabIndexGlobal(5);
          this.mainclassref.setTabIndexEntregados(0);
  ...
  aprobarPedido(element){
    this.http2.post(Urlbase.facturacion+"/pedidos/actualizarEstadoPedidoApp?idbillpedidoprov="+element.id_BILL+"&idbillstateclient=704&idbillstateprov=804&notas=ENTREGADO SIN NOVEDAD"+"&ESTADOORIGENPROV="+this.SelectedBillState+"&IDTHIRDUSER="+this.locStorage.getToken().id_third+"&ACTOR=P",{}).subscribe(response => {
      this.showNotification('top', 'center', 3, "<h3>Se ENTREGO su pedido con <b>EXITO</b></h3> ", 'info');
      this.mainclassref.SendFCMpush("Pedido Entregado","Tu pedido ha sido 'Entragado'",{message:"Order Update",id_bill:element.id_BILL,status:704,status_p:804},element.idthirdcliente,element.cliente,element.idapp);
      this.mainclassref.compUpdate5.updateData();
      this.mainclassref.compUpdate6.updateData();
      this.mainclassref.compUpdate7.updateData();
      this.mainclassref.compUpdate8.updateDataRecibidos()
      ...
      this.ngOnInit();
      this.mainclassref.setTabIndexGlobal(5);
      this.mainclassref.setTabIndexEntregados(1);
