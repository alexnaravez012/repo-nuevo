...
@Component({
  selector: 'app-pedidos-domicilios-entregados-s',
  templateUrl: './pedidos-domicilios-entregados-s.component.html',
  styleUrls: ['./pedidos-domicilios-entregados-s.component.css']
})
export class PedidosDomiciliosEntregadosSComponent implements OnInit {
  @Input()
  ...
  updateData(){
    ...
    this.http2.get(Urlbase.facturacion+ "/pedidos/getDatosDomiciliario?id_store="+this.locStorage.getIdStore()+"&status=R").subscribe(item => {
      CPrint("domi: ")
      CPrint(item);
      this.listDomici = item;
      this.selectedDomi = item[0];
  ...
  //-TIMER-//
  this.intervalUpdate = setInterval(function(main:PedidosDomiciliosEntregadosSComponent){
    main.UpdateTimer();
  },1000,this);
  //-TIMER-//
}
...
//-TIMER-//
  ...
  filterList;
  updateButtonStatus(){
    this.http2.get(Urlbase.facturacion+ "/pedidos/getIdBillPlanilla?id_store="+this.locStorage.getIdStore()+"&id_planilla="+this.selectedDomi.id_PLANILLA).subscribe(result => {
      this.filterList=result;
      console.log(this.filterList);
      this.dataSource.filter = this.filterList.join(",");
    })

    this.http2.get(Urlbase.facturacion+"/pedidos/getPlanillasData/cierreCaja?id_cierre_caja="+this.selectedDomi.id_CIERRE_CAJA+"&status=C").subscribe(resp => {
      CPrint("resp is this")
      CPrint(resp)
      //@ts-ignore
      let answer = resp.filter( object=> this.selectedDomi.id_PLANILLA === object.id_PLANILLA )
      if(answer.length>0){
        this.enableButton = true;
      }else{
  ...
  cerrarPlanilla(){
    ...
    const dialogRef = this.dialog.open(PedidosDomiciliosCerrarPlanillasComponent, {
      width: '120vw',
      height: '80vh',
      data: {idPlanilla: this.selectedDomi.id_PLANILLA,
        blockedObs: false,
