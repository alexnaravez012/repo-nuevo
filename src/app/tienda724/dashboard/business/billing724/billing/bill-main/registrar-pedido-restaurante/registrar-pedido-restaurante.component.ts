...
@Component({
  selector: 'app-registrar-pedido-restaurante',
  templateUrl: './registrar-pedido-restaurante.component.html',
  styleUrls: ['./registrar-pedido-restaurante.component.css']
})
export class RegistrarPedidoRestauranteComponent implements OnInit {
  ...
  constructor(private authService:AuthenticationService, private service: StoreSelectorService,public router: Router, private datePipe: DatePipe, public thirdService: ThirdService,private http: HttpClient, public locStorage: LocalStorage,private fb: FormBuilder, private billingService: BillingService,
    ...
    this.billDTO = new BillDTO(this.idStoreForUse,null, null, null, null, null, null, null, null, null, null, null, null, this.commonStateDTO, null, this.detailBillDTOList, this.documentDTO);
    this.createControls();
  }
  currentBox;
  idThird;
  ...
  setidStoreForUse(id){
    this.idStoreForUse = id;
  }

  getidStoreForUse(){
    return this.idStoreForUse;
  }
      ...
      if (res.length == 0){
        ...
        dialogRef.afterClosed().subscribe(result => {
          if(result){
            ...
            this.http.get(Urlbase.facturacion + "/billing/pedidos?id_store="+this.idStoreForUse+"&id_third="+aux.id_PERSON).subscribe(r => {
              //@ts-ignore
              r.push({numdocumento: "",
              id_BILL: -1,
              id_THIRD: -1})
  ...
  setTable(item){
    console.log("SETTING TABLE: ", item);
    this.mesaSeleccionada = item.id_MESA;
    this.mesaName = item.mesa;
  }
  ...
  openMesaDisabled = false;
  quickConfirm(){
    this.openMesaDisabled = true;
    ...
    this.productsObject.forEach(item => {
      for(let i = 0; i<item.quantity; i++){
          detailListNew = detailListNew+ "{"+item.id_product_third+",0},"
      }
    });



    CPrint(Urlbase.facturacion+ "/billing/crearPedidoMesa?idstoreclient=11&idthirduseraapp="+this.locStorage.getToken().id_third+"&idstoreprov="+this.idStoreForUse+"&detallepedido="+detailList.substring(0, detailList.length - 1)+"&descuento="+0+"&idpaymentmethod="+1+"&idapplication="+40+"&idthirdemp="+this.selectedDomi+"&detallepedidomesa="+detailListNew.substring(0, detailListNew.length - 1)+"&idmesa="+this.mesaSeleccionada);
    this.http.post(Urlbase.facturacion+ "/billing/crearPedidoMesa?idstoreclient=11&idthirduseraapp="+this.locStorage.getToken().id_third+"&idstoreprov="+this.idStoreForUse+"&detallepedido="+detailList.substring(0, detailList.length - 1)+"&descuento="+0+"&idpaymentmethod="+1+"&idapplication="+40+"&idthirdemp="+this.selectedDomi+"&detallepedidomesa="+detailListNew.substring(0, detailListNew.length - 1)+"&idmesa="+this.mesaSeleccionada,{}).subscribe(item => {
      if(item==1){
        this.showNotification('top', 'center', 3, "<h3>Pedido creado con <b>EXITO.</b></h3> ", 'info');
        this.cancel();
        this.openMesaDisabled = false;
        this.mainclassref.compUpdate.ngOnInit();
        this.mainclassref.setTabIndexGlobal(0);
      }else{

        console.log("URL: "+Urlbase.facturacion+ "/billing/crearPedidoMesa?idstoreclient=11&idthirduseraapp="+this.locStorage.getToken().id_third+"&idstoreprov="+this.idStoreForUse+"&detallepedido="+detailList.substring(0, detailList.length - 1)+"&descuento="+0+"&idpaymentmethod="+1+"&idapplication="+40+"&idthirdemp="+this.selectedDomi+"&detallepedidomesa="+detailListNew.substring(0, detailListNew.length - 1)+"&idmesa="+this.mesaSeleccionada);
        this.openMesaDisabled = false;
        this.showNotification('top', 'center', 3, "<h3>Se presento un error al crear el pedido.</h3> ", 'danger');
        this.cancel();
      }
    })
  }
  ...
  sendData(data){
    ...
    if(this.id_person){
      ...
      this.billDTO.id_store = this.idStoreForUse;
  ...
  async DelayedgetInventoryList(){
    ...
    while(1==1){
      ...
      if(this.idStoreForUse != 1){break;}
    }
    if(this.locStorage.getBoxStatus()){
      await this.getInventoryList(this.idStoreForUse)
    };
  ...
  selectedDomi;
  ngOnInit() {
    this.httpClient.get(Urlbase.facturacion + "/billing/domiciliarios?id_store="+this.idStoreForUse).subscribe(data => {
      this.domiList = data;
    ...
    CPrint("ON VENTA");
                ...
                let dialogRef = this.dialog.open(Openorclosebox2Component, {
                  ...
                  if(response.resp){
                    ...
                    this.http.get(Urlbase.cierreCaja+"/close/myboxes?id_person="+this.locStorage.getPerson().id_person).subscribe(resp => {
                      ...
                      this.SelectedStore=this.idStoreForUse;
                        ...
                        if(response2.logout){
                          ...
                          dialogRef = this.dialog.open(OpenBoxComponent, {
                            ...
                            this.http.post(Urlbase.facturacion+"/pedidos/abrirCajaPlanilla?IDCIERRECAJA="+this.locStorage.getIdCaja(),{}).subscribe(e=> {
                              ...
                              CPrint("ID STORE: ",this.idStoreForUse);
                  ...
                  let dialogRef = this.dialog.open(SelectboxComponent, {
                    ...
                    if(response2.logout){
                      ...
                      dialogRef = this.dialog.open(OpenBoxComponent, {
                        ...
                        this.http.post(Urlbase.facturacion+"/pedidos/abrirCajaPlanilla?IDCIERRECAJA="+this.locStorage.getIdCaja(),{}).subscribe(e=> {
                          this.service.onMainEvent.emit(this.idStoreForUse);
                          this.locStorage.setBoxStatus(true);
                          CPrint("ID CAJA: ",this.locStorage.getIdCaja());
                          CPrint("ID STORE: ",this.idStoreForUse);
                          CPrint("STORE TYPE: ",this.locStorage.getTipo());
      ...
      }else{this.getStoreType(this.idStoreForUse);}
  ...
  addDetail2(code) {
    console.log("AT THE MOMENT ID IS: ", this.idStoreForUse);
    console.log("TE INVETORY AT THE MOMENT IS: ", this.inventoryList);
  ...
  addDetailExterno(code) {


    console.log("AT THE MOMENT ID IS: ", this.idStoreForUse);
    console.log("TE INVETORY AT THE MOMENT IS: ", this.inventoryList);
    this.inventoriesService.getInventory(this.idStoreForUse).subscribe((data: InventoryName[]) => {
      this.inventoryList = data;

      console.log("TE INVETORY AT THE SECOND MOMENT IS: ", this.inventoryList);
  ...
  getStorages() {
    this.httpClient.get(Urlbase.tienda+"/store/s?id_store="+this.idStoreForUse).subscribe((res)=>{
  ...
  cancel2(){
    ...
    this.mesaName = "Sin Mesa";
    this.mesaSeleccionada = "-1";
    this.id_person=3088;
    this.pedidosList=[];
    ...
    if(isDisc == 1 || isDisc == 4){
      ...
      try{
        inventoryQuantityDTOList.forEach(element =>{
          this.inventoriesService.putQuantity(this.idStoreForUse,(-1)*Math.floor(element.quantity),element.code,inventoryQuantityDTOList,isDisc,element.id_storage).subscribe(() =>{
          });
        });
        this.showNotification('top', 'center', 3, "<h3>Se ha realizado el movimento <b>CORRECTAMENTE</b></h3> ", 'info');
        this.cancel();
