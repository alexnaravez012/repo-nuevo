...
let $: any = jQuery;
@Component({
  selector: 'app-modal-crear-pedidos',
  templateUrl: './modal-crear-pedidos.component.html',
  styleUrls: ['./modal-crear-pedidos.component.css']
})
export class ModalCrearPedidosComponent implements OnInit {
  tax=0;
  price = 0;
  cost = 0;
  Stores = [];
  ...
  listTable;
  storeIva ="";
  constructor(public dialogRef: MatDialogRef<ModalCrearPedidosComponent>
    ,@Inject(MAT_DIALOG_DATA) public data: any,private categoriesService: BillingService, public dialog: MatDialog, private billingService : BillingService,private http: HttpClient, public locStorage: LocalStorage,public router: Router,) { 
  ...
  ngOnInit() {
    if(this.data.flag){
      this.lockStuff = true;
      this.id_person = this.data.id_third;
      this.cliente = this.data.third_name;
      this.SelectedStore = this.data.id_store_prov;
      this.ccClient = this.data.id_third_document;
    }
    this.getIva();
