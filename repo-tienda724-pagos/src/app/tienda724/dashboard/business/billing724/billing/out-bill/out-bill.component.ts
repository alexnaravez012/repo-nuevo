...
@Component({
  selector: 'out-bill-main',
  templateUrl: './out-bill.component.html',
  styleUrls: ['./out-bill.component.scss']
})
export class OutBillComponent implements OnInit {
  ...
  addDetail(event) {
    ...
    if (key != null) {
      ...
      CPrint("this is price product: ", this.priceList);
       //@ts-ignore
       if(product){
        this.getPriceList2(product,code,product.id_PRODUCT_STORE)
      }else{
  ...
  calculateSubtotal() {
    ...
    CPrint(this.productsObject,"this is products object");
  ...
  sendData(data,disc,isCompra){
    ...
    if (this.detailBillDTOList.length > 0) {
      ...
      if(disc==3 || disc==4){
        this.billDTO.subtotal = this.calculateSubtotal();
        this.billDTO.tax = this.calculateSubtotal();
        this.billDTO.totalprice = this.calculateSubtotal();
      }
