...
import { PedidosVentaConfirmarComponent } from '../pedidos-venta-confirmar/pedidos-venta-confirmar.component';
import { ConfirmacionEnvioDomicilioComponent } from '../confirmacion-envio-domicilio/confirmacion-envio-domicilio.component';
import { PedidoConfirmacionCancelacionComponent } from '../pedido-confirmacion-cancelacion/pedido-confirmacion-cancelacion.component';
import { LogPedidosComponent } from '../log-pedidos/log-pedidos.component';
import { ModalNoDomiComponent } from '../modal-no-domi/modal-no-domi.component';
import { ObservationsNotesComponent } from '../observations-notes/observations-notes.component';

let $: any = jQuery;

@Component({
  selector: 'app-pedidos-domicilios-procesados-s',
  templateUrl: './pedidos-domicilios-procesados-s.component.html',
  styleUrls: ['./pedidos-domicilios-procesados-s.component.css']
})
export class PedidosDomiciliosProcesadosSComponent implements OnInit {
  @Input()
  ...
  idMenu;
  ngOnInit() {
    this.idMenu = this.locStorage.getMenu().filter(element => element.id_menu == 143 || element.id_menu == 281);
    console.log(this.locStorage.getMenu())
  ...
  updateData(){
    ...
    //-TIMER-//
    this.intervalUpdate = setInterval(function(main:PedidosDomiciliosProcesadosSComponent){
      main.UpdateTimer();
    },1000,this);
    //-TIMER-//
  }
  ...
  //-TIMER-//
  segs:{[key:number]:number} = {};

  UpdateTimer(){
    for(let n = 0;n<this.dataSource.data.length;n++){
      let element:any = this.dataSource.data[n];

      let a = new Date(element.fecha)
      let b = new Date();


      this.segs[element.id_BILL] = parseInt(((b.getTime() - a.getTime())/60000)+"")
    }
  }
  openObs(obs){
  ...
  cancelarPedido(element){
    const dialogRef = this.dialog.open(PedidoConfirmacionCancelacionComponent, {
      height: '38vh',
      width: '80vw',
      disableClose: true
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      CPrint(result)
      if(result.response){
        this.http2.post(Urlbase.facturacion+"/pedidos/actualizarEstadoPedidoApp?idbillpedidoprov="+element.id_BILL+"&idbillstateclient=99&idbillstateprov=99&notas=CANCELADO: "+result.obs+"&ESTADOORIGENPROV="+this.SelectedBillState+"&IDTHIRDUSER="+this.locStorage.getToken().id_third+"&ACTOR=P",{}).subscribe(response => {
          this.showNotification('top', 'center', 3, "<h3>Se CANCELO su pedido con <b>EXITO</b></h3> ", 'info');
          this.mainclassref.SendFCMpush("Pedido Cancelado","Tu pedido ha sido 'Cancelado'",{message:"Order Update",id_bill:element.id_BILL,status:99,status_p:99},element.idthirdcliente,element.cliente,element.idapp);
          this.mainclassref.compUpdate.updateData();
          this.ngOnInit()
          this.mainclassref.setTabIndexGlobal(8);
  ...
  aprobarPedido(element){
    this.http2.post(Urlbase.facturacion+"/pedidos/actualizarEstadoPedidoApp?idbillpedidoprov="+element.id_BILL+"&idbillstateclient=702&idbillstateprov=802&notas=PROCESADO SIN NOVEDAD"+"&ESTADOORIGENPROV="+this.SelectedBillState+"&IDTHIRDUSER="+this.locStorage.getToken().id_third+"&ACTOR=P",{}).subscribe(response => {
      this.showNotification('top', 'center', 3, "<h3>Se PROCESO su pedido con <b>EXITO</b></h3> ", 'info');
      this.mainclassref.SendFCMpush("Pedido En Proceso","Tu pedido ahora está en 'Proceso'",{message:"Order Update",id_bill:element.id_BILL,status:702,status_p:802},element.idthirdcliente,element.cliente,element.idapp);
      this.mainclassref.compUpdate3.updateData();
      this.ngOnInit();
      this.mainclassref.setTabIndexGlobal(2);
      this.mainclassref.setTabIndexProcesados(1);
  ...
  aprobarPedidoConNovedad(elem){

    console.log(elem);

    const dialogRef = this.dialog.open(PedidosVentaConfirmarComponent, {
      width: '120vw',
      height: '80vh',
    ...
    dialogRef.afterClosed().subscribe(result => {
      CPrint(result)
      if(result.response){
        this.mainclassref.SendFCMpush("Pedido Con Novedad","Tu pedido ha presentado una novedad'",{message:"Order Update",id_bill:elem.id_BILL,status:707,status_p:807},elem.idthirdcliente,elem.cliente,elem.idapp);
        this.mainclassref.compUpdate4.updateData();
        this.mainclassref.setTabIndexGlobal(2);
        this.mainclassref.setTabIndexProcesados(0);
        this.updateData();
      }else{
        CPrint("I FAILED")
      }
  ...
  enviarPedido(elem){
    ...
    this.http2.get(Urlbase.facturacion + "/pedidos/getThirDataPedidos?id_store="+this.locStorage.getIdStore()).subscribe(data => {
      //@ts-ignore
      if(data.length>0){

        const dialogRef = this.dialog.open(ConfirmacionEnvioDomicilioComponent, {
          width: '120vw',
          height: '80vh',
          data: { element: elem },
          disableClose: true
        ...
        dialogRef.afterClosed().subscribe(result => {
          CPrint(result)
          if(result.response){
            this.http2.post(Urlbase.facturacion+"/pedidos/enviarPedidoApp?idbillpedidoprov="+elem.id_BILL+"&idbillventa="+result.idBill+"&notes="+result.obs+"&ESTADOORIGENPROV="+this.SelectedBillState+"&IDTHIRDUSER="+this.locStorage.getToken().id_third+"&ACTOR=P",{}).subscribe(response => {
              this.mainclassref.SendFCMpush("Pedido Facturado","Tu pedido ha sido 'Facturado'",{message:"Order Update",id_bill:elem.id_BILL,status:901,status_p:902},elem.idthirdcliente,elem.cliente,elem.idapp);
              this.mainclassref.compUpdate9.updateData();
              this.mainclassref.compUpdate8.updateDataRecibidos();
              this.mainclassref.setTabIndexGlobal(3);
              this.updateData();
            })
          }else{
            CPrint("I FAILED")
        ...
        const dialogRef = this.dialog.open(ModalNoDomiComponent, {
          width: '40vw',
          height: '25vh',
          data: { element: elem }
        });
  ...
  getRepProdList(){
    ...
    this.DiccionarioColumnas["timer"] = "Tiempo Transcurrido";this.AnchosColumnas["timer"] = "4vw";
