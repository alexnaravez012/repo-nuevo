...
@Component({
  selector: 'app-pedidos-domicilios-procesados-n',
  templateUrl: './pedidos-domicilios-procesados-n.component.html',
  styleUrls: ['./pedidos-domicilios-procesados-n.component.css']
})
export class PedidosDomiciliosProcesadosNComponent implements OnInit {
  @Input()
  ...
  updateData(){
    ...
    CPrint("THIS IS MY ID STORE: ",this.id_store)
     //-TIMER-//
     this.intervalUpdate = setInterval(function(main:PedidosDomiciliosProcesadosNComponent){
      main.UpdateTimer();
    },1000,this);
    //-TIMER-//
  }
  ...
  aprobarPedido(element){
    this.http2.post(Urlbase.facturacion+"/pedidos/actualizarEstadoPedidoApp?idbillpedidoprov="+element.id_BILL+"&idbillstateclient=702&idbillstateprov=802&notas=SIN NOVEDAD"+"&ESTADOORIGENPROV="+this.SelectedBillState+"&IDTHIRDUSER="+this.locStorage.getToken().id_third+"&ACTOR=P",{}).subscribe(response => {
