...
import { PedidosDomiciliosPlanillasComponent } from '../pedidos-domicilios-planillas/pedidos-domicilios-planillas.component';
...
@Component({
  ...
  styleUrls: ['./pedidos-facturados.component.css']
})
export class PedidosFacturadosComponent implements OnInit {
  ...
  updateDataRecibidos(){
    ...
    this.http2.get(Urlbase.facturacion+ "/pedidos/getPlanillas?id_store="+this.locStorage.getIdStore()+"&status=O").subscribe(item => {
      CPrint("PLANILLAS: ")
      CPrint(item);
      this.listPlanillas = item;
      this.selectedPlanilla=item[0].id_PLANILLA;
    ...
    CPrint("THIS IS MY ID STORE: ",this.id_store)
     //-TIMER-//
     this.intervalUpdate = setInterval(function(main:PedidosFacturadosComponent){
      main.UpdateTimer();
    },1000,this);
    //-TIMER-//
  }
  ...
  openPlanilla(){
    ...
    const dialogRef = this.dialog.open(PedidosDomiciliosPlanillasComponent, {
      width: '120vw',
      height: '80vh',
      data: {idPlanilla: this.selectedPlanilla}
    });
  ...
  filterTable(){
    console.log()
    this.http2.get(Urlbase.facturacion+ "/pedidos/getIdBillPlanilla?id_store="+this.locStorage.getIdStore()+"&id_planilla="+this.selectedPlanilla).subscribe(result => {
      this.filterList=result;
  ...
  sendPlanillas(){
    this.http2.post(Urlbase.facturacion+ "/pedidos/enviarPedidosFacturados?idplanilla="+this.selectedPlanilla+"&ESTADOORIGENPROV="+this.SelectedBillState+"&IDTHIRDUSER="+this.locStorage.getToken().id_third+"&ACTOR=P",{}).subscribe(item => {
      CPrint(item);
      if(item==1){
        //let Selection = Object.keys(this.DictSelection);
        console.log("Selection es ")
        console.log(this.dataSource.data)
        for(let n = 0;n<this.ListReportProd.length;n++) {
          let item:any = this.dataSource.data[n];
          this.mainclassref.SendFCMpush("Pedido En Camino","Tu pedido ya ha sido enviado",{message:"Order Update",id_bill:item.id_BILL,status:703,status_p:803},item.idthirdcliente,item.cliente,item.idapp);
        }
        this.showNotification('top', 'center', 3, "<h3>Se procesaron los pedidos de la planilla con exito.</h3>", 'info',9999999999999999999999999);
        ...
        this.mainclassref.setTabIndexGlobal(4);

      }else{
        this.showNotification('top', 'center', 3, "<h3>Algo fallo al procesar los pedidos.</h3>", 'danger');
      }
      this.ngOnInit();
    })
  }
    ...
    for(let n = 0;n<Selection.length;n++){
      ...
      let respuesta = await new Promise(resolve => {
        this.http2.post(Urlbase.facturacion + "/pedidos/sendPedidosV2?idbpedido="+elemToPost.id_BILL+"&idstoreprov="+elemToPost.id_STORE+"&idstoreclient="+elemToPost.id_STORE_CLIENT+"&idplanilla="+this.selectedPlanilla,{}).subscribe(idBillVent => {
          if(idBillVent!=0){
            this.mainclassref.SendFCMpush("Pedido En Camino","Tu pedido ya ha sido enviado",{message:"Order Update",id_bill:elemToPost.id_BILL,status:703,status_p:803},elemToPost.idthirdcliente,elemToPost.cliente,elemToPost.idapp);
  ...
  cancelarPedido(element){
    ...
    dialogRef.afterClosed().subscribe(result => {
      CPrint(result)
      if(result.response){
        this.http2.post(Urlbase.facturacion+"/pedidos/cancelarPedidoApp?idbillpedidoprov="+element.id_BILL+"&notes="+result.obs+"&idcaja="+this.locStorage.getIdCaja()+"&ESTADOORIGENPROV="+this.SelectedBillState+"&IDTHIRDUSER="+this.locStorage.getToken().id_third+"&ACTOR=P",{}).subscribe(response => {
